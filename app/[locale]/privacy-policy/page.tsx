      ...
      <main className="flex-grow container max-w-4xl mx-auto px-4 lg:px-6 py-16 lg:py-24">
        ...
        >
          ...
          <ul className="list-disc list-inside space-y-2 mb-8 text-gray-700">
            {(t.raw('principlesList', { default: [] }) as string[]).map((item, idx) => <li key={idx}>{item}</li>)}
          </ul>
          ...
          <ul className="list-disc list-inside space-y-2 mb-8 text-gray-700">
            {(t.raw('studentsList', { default: [] }) as string[]).map((item, idx) => <li key={idx}>{item}</li>)}
          </ul>
          ...
          <ul className="list-disc list-inside space-y-2 mb-8 text-gray-700">
            {(t.raw('useList', { default: [] }) as string[]).map((item, idx) => <li key={idx}>{item}</li>)}
          </ul>

          <p className="text-gray-700 mb-6">{t('useText2')}</p>
          <ul className="list-disc list-inside space-y-2 mb-8 text-gray-700">
            {(t.raw('useList2', { default: [] }) as string[]).map((item, idx) => <li key={idx}>{item}</li>)}
          </ul>
